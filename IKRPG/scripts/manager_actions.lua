...
function handleActionDrop(draginfo, rTarget)
	if rTarget then
		...
		-- NOTE: Need to update targeting handlers to allow triggers on targeting, regardless of whether target is already specified.  (i.e. Marked condition and attacks)
		if sDragType == "attack" then
			if EffectsManager.applyMarkPenalty(rSource, { rTarget }) then
