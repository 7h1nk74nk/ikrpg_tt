...
function evalDice(aDice, nMod, bCritical, bVorpal, nBrutal)
	local nTotal = 0;
	for _,sDie in pairs(aDice) do
		...
		if nDieSides > 0 then
			if bCritical then
				...
				local bRollAgain = true;
				while bRollAgain do
					bRollAgain = false;
					local nRoll = math.random(nDieSides);
					if nBrutal then
						if nBrutal + 1 >= nDieSides then
							nRoll = nDieSides;
						else
							while nRoll <= nBrutal do
								nRoll = math.random(nDieSides);
							end
						end
					end
					nTotal = nTotal + nRoll;
					if bVorpal and nRoll == nDieSides then
						bRollAgain = true;
					end
				end
			end
