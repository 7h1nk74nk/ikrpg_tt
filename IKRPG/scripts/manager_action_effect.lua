...
function encodeEffect(rAction)
	...
	rRoll.aDice = {};
	rRoll.nMod = rAction.nSaveMod or 0;
...
function encodeEffectAsText(rEffect)
	...
	if rEffect then
		...
		local sExpire = "[EXPIRES";
		if rEffect.sExpire ~= "" then
			sExpire = sExpire .. " " .. rEffect.sExpire;
		end
		sExpire = sExpire .. "]";
		table.insert(aMessage, sExpire);
...
function decodeEffectFromText(sEffect)
	...
	if sEffectName then
		...
		rEffect.sExpire = string.match(sEffect, "%[EXPIRES ?([^]]+)%]") or "";
